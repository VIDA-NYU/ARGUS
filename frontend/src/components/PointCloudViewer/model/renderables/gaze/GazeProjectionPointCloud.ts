...
export class GazeProjectionPointCloud extends PointCloud {
    ...
    public initialize_highlights(): void {
        ...
        // Point highlight
        const destinationColor: THREE.Color = new THREE.Color( BASE_COLORS['gazeorigin-pointcloud'][0], BASE_COLORS['gazeorigin-pointcloud'][1], BASE_COLORS['gazeorigin-pointcloud'][2] );
        ...
        // Direction highlight
        const lineMaterial = new THREE.LineDashedMaterial({ color: originColor, linewidth: 5, scale: 0.1, dashSize: 0.01, gapSize: 0.3, });
    ...
    public highlight( raycaster: Raycaster, dataset: Dataset, intersects: any[] ): number {
        ...
        const pointProjection: number[] = dataset.pointClouds['gazeorigin-pointcloud'].points[pointIndex];
